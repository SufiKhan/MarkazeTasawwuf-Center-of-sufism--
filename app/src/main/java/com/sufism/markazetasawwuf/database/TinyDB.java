...
package com.sufism.markazetasawwuf.database;
...
public class TinyDB {
    ...
    public ArrayList<QBFile> getQbFiles(String key) {
    	String[] myList = TextUtils.split(preferences.getString(key, ""), "â€šâ€—â€š");
        ArrayList<String> arrayToList = new ArrayList<String>(Arrays.asList(myList));
        ArrayList<QBFile> newList = new ArrayList<QBFile>();
        for (String file : arrayToList)
            newList.add(new QBFile(file));
        return newList;
    }
    ...
    // edited by ME for QBFile
    
    public void putListQBFile(String key, ArrayList<QBFile> qbList) {
        preferences.edit().putString(key, TextUtils.join("‚‗‚", qbList)).apply();
    }
    /**
